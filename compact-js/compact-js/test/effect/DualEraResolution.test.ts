...
describeWithFixtures('two ledger eras in one process', () => {
  ...
  it('executes a circuit on each era, in the same process', async () => {
    ...
    // Era 9: call-tree model, circuit id first.
    ...
    const era9Result = await era9Contract.provableCircuits.increment!(
      (era9Runtime.createCircuitContext as (...a: unknown[]) => unknown)(
        'increment',
        era9Runtime.sampleContractAddress(),
        era9Runtime.emptyZswapLocalState(coinPublicKey),
        era9Initial.currentContractState,
        era9Initial.currentPrivateState
      ) as never
    );
