 ...
 * requirement in #388.
 *
 * **`ContractExecutable`, `CompiledContract` and the configuration services are absent for a
 * different reason: an outstanding implementation limit, not an era truth.** Those modules import
 * the `Ledger` and `CompactRuntime` facades by module path, so they resolve whatever `current.ts`
 * binds — today ledger 9. Exposing them here would hand back ledger-9-bound objects from a path
 * named v8, which is exactly the mis-labelling this entry exists to avoid. Ledger 8 execution
 * itself is proven to work (`test/era8/LedgerEightExecution.test.ts` runs a circuit end to end);
 * what remains is parameterising those modules on an era pair the way the conversions now are.
 *
