...
for (const pkg of packages) {
  ...
  const tarball = tarballs.find((f) => f.includes(pkg.version)) ?? (tarballs.length === 1 ? tarballs[0] : null);
  if (!tarball) {
    console.error(
      `release: could not disambiguate tarball for ${pkg.name}@${pkg.version} in ${distDir}: ${tarballs.join(', ')}`
    );
    process.exit(1);
  }
